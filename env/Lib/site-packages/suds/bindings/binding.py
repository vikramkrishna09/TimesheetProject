...
class Binding:
    ...
    def get_reply(self, method, reply):
        ...
        result_head=None
        # parse header
        soaphead = soapenv.getChild('Header')
        nodes = self.replyheader(method, soaphead)
        rtypes = self.returned_head_types(method)
        if len(rtypes) > 1:
            result_head = self.replycomposite(rtypes, nodes, name='head')
        elif len(rtypes) == 1:
            if rtypes[0].unbounded():
                result_head = self.replylist(rtypes[0], nodes)
            elif len(nodes):
                unmarshaller = self.unmarshaller()
                resolved = rtypes[0].resolve(nobuiltin=True)
                result_head = unmarshaller.process(nodes[0], resolved)
        # parse body
    ...
    def replycomposite(self, rtypes, nodes, name='reply'):
        """
